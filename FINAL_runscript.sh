...
# Create directories for each tool output
...
if ! check_for_checkpoint "medaka"; then
    ...
    medaka_consensus -i $DATA_OUTPUT_PATH/medaka/work_files/consensus.fasta -d $DATA_OUTPUT_PATH/flye/assembly.fasta -o $DATA_OUTPUT_PATH/medaka    create_checkpoint "medaka"
...
if ! check_for_checkpoint "kraken2"; then
    ...
    if [[ -z "$DBNAME" ]]; then
        ...
        #export database location to config
